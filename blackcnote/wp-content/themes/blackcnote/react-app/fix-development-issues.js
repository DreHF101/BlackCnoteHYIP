...
startServices().catch(console.error);
  ...
  fs.writeFileSync(path.join(__dirname, 'dev-enhanced.js'), devScript);
  fs.chmodSync(path.join(__dirname, 'dev-enhanced.js'), '755');
  
  // Update package.json scripts
  const packageJsonPath = path.join(__dirname, 'package.json');
  const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf8'));
  
  packageJson.scripts['dev:enhanced'] = 'node dev-enhanced.js';
  packageJson.scripts['dev:full:fixed'] = 'node dev-enhanced.js';
