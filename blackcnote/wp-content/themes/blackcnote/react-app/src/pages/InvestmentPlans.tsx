...
import { fetchPlans, WPContent, fetchSettings } from '../api/wordpress';
...
const InvestmentPlans = () => {
  ...
  const [wpPlans, setWpPlans] = useState<WPContent[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchPlansData = async () => {
      try {
        ...
        setPlans(plans.map(plan => ({
          id: plan.id,
          name: plan.title,
          return_rate: plan.return_rate ? Number(plan.return_rate) : 0,
          min_investment: plan.min_investment ? Number(plan.min_investment) : 0,
          max_investment: plan.max_investment ? Number(plan.max_investment) : 0,
          duration: plan.duration ? Number(plan.duration) : 0,
    ...
    if (liveEditingEnabled) {
      interval = setInterval(fetchPlansData, 5000);
    }
    return () => {
