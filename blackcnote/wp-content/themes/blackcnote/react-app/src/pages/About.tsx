...
const About = () => {
  const [liveEditingState, liveEditingActions] = useLiveEditing({
    onChange: (change) => {
      // Handle content/component changes for About page
      if (change.type === 'content' && change.id === 'about-page') {
        // Optionally update state if About content is editable
        // setAboutContent(change.content);
      }
    }
  });
