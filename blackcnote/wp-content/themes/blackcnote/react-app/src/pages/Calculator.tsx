...
const Calculator = () => {
  ...
  const [liveEditingState, liveEditingActions] = useLiveEditing({
    onChange: (change) => {
      // Handle content/component changes for Calculator page
      if (change.type === 'component' && change.name === 'calculator-plans') {
        setPlans(change.data);
      }
      if (change.type === 'component' && change.name === 'calculator-results') {
        setResults(change.data);
      }
    }
  });
