                    ...
                    <table class="table table--light style--two custom-data-table">
                        <thead>
                            <tr>
                                ...
                                <th><?php esc_html_e('Supported Currency', HYIPLAB_PLUGIN_NAME); ?></th>
                                <th><?php esc_html_e('Enabled Currency', HYIPLAB_PLUGIN_NAME); ?></th>
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td><?php echo hyiplab_gateway_currency_count($gateway->code); ?></td>
                                    <td>
                                        <?php
                                        if ($gateway->status == 1) {
                                    ...
                                    <td>
                                        <div class="button--group">
                                            <a href="<?php echo hyiplab_route_link('admin.gateway.automatic.edit'); ?>&amp;id=<?php echo intval($gateway->id); ?>" class="btn btn-sm btn-outline--primary">
                                                <i class="la la-pencil"></i> <?php esc_html_e('Edit', HYIPLAB_PLUGIN_NAME); ?>
                                            </a>
                                            <?php if ($gateway->status == 0) { ?>
                                                <button class="btn btn-sm btn-outline--success ms-1 confirmationBtn" data-question="<?php esc_attr_e('Are you sure to enable this gateway?', HYIPLAB_PLUGIN_NAME); ?>" data-action="<?php echo hyiplab_route_link('admin.gateway.automatic.status'); ?>&amp;id=<?php echo intval($gateway->id); ?>" data-nonce="<?php echo esc_attr(hyiplab_nonce('admin.gateway.automatic.status'));?>">
                                                    <i class="la la-eye"></i> <?php esc_html_e('Enable', HYIPLAB_PLUGIN_NAME); ?>
                                                </button>
                                            <?php } else { ?>
                                                <button class="btn btn-sm btn-outline--danger ms-1 confirmationBtn" data-question="<?php esc_attr_e('Are you sure to disable this gateway?', HYIPLAB_PLUGIN_NAME); ?>" data-action="<?php echo hyiplab_route_link('admin.gateway.automatic.status'); ?>&amp;id=<?php echo intval($gateway->id); ?>" data-nonce="<?php echo esc_attr(hyiplab_nonce('admin.gateway.automatic.status'));?>">
