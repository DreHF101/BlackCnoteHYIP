...
function Set-DockerDesktopSettings {
    ...
    try {
        ...
        Write-ColorOutput "Error configuring Docker settings: $($_.Exception.Message)" "Yellow"
        return $false
    }
}
...
function Set-DockerDaemonConfig {
    ...
    try {
        ...
        Write-ColorOutput "Error configuring Docker daemon: $($_.Exception.Message)" "Yellow"
        return $false
    }
}
...
function Stop-DockerDesktop {
    ...
    try {
        ...
        if ($dockerProcesses) {
            $dockerProcesses | ForEach-Object {
                try {
                    ...
                    Write-ColorOutput "Could not stop process $($_.ProcessName)" "Yellow"
                }
            }
        }
        ...
        Write-ColorOutput "Error stopping Docker processes: $($_.Exception.Message)" "Yellow"
    }
}
...
function Start-DockerDesktop {
    ...
    try {
        ...
        $dockerProcesses = Get-Process -Name "*docker*" -ErrorAction SilentlyContinue
        if ($dockerProcesses) {
            Write-ColorOutput "Docker Desktop processes already running. Stopping them first..." "Yellow"
            Stop-DockerDesktop
...
function Clear-DockerDesktopData {
    ...
    try {
        ...
        Write-ColorOutput "Error cleaning Docker data: $($_.Exception.Message)" "Yellow"
        return $false
    }
}
    ...
    try {
        ...
        while ($retryCount -lt $maxRetries) {
            try {
                ...
                $retryCount++
                Write-ColorOutput "Attempt $retryCount failed: $($_.Exception.Message)" "Yellow"
...
function Get-ServiceStatus {
    ...
    try {
        ...
        Write-ColorOutput "Error checking service status: $($_.Exception.Message)" "Yellow"
        return $false
    }
}
...
Write-ColorOutput "Version: 2.0.1 - Simplified without emojis" "Gray"
