...
function blackcnote_theme_scripts(): void {
    ...
    wp_enqueue_script(
        ...
        get_template_directory_uri() . '/assets/js/blackcnote.js',
        ['jquery', 'bootstrap'],
        wp_get_theme()->get('Version'),
        true
    );
...
function blackcnote_is_plugin_active() {
    return function_exists('blackcnote_system_instance');
}
...
function blackcnote_theme_calculate_return(): void {
    ...
    // Get plan details from BlackCnote
    global $wpdb;
    $plan = $wpdb->get_row($wpdb->prepare(
        "SELECT * FROM {$wpdb->prefix}blackcnote_plans WHERE id = %d",
        $plan_id
...
function blackcnote_theme_filter_transactions(): void {
    ...
    // Build query
    global $wpdb;
    $query = "SELECT * FROM {$wpdb->prefix}blackcnote_transactions WHERE user_id = %d";
