<?php
/**
 * Override template for BlackCnote dashboard
 *
 * This template overrides the default BlackCnote dashboard view
 * with custom styling and additional features.
 *
 * @package BlackCnote
 * @subpackage BlackCnote
 */
...
if (!$user->exists()) {
    wp_die(__('You must be logged in to view this page.', 'blackcnote'));
}
...
$investments = $wpdb->get_results(
    $wpdb->prepare(
        "SELECT * FROM {$wpdb->prefix}blackcnote_investments WHERE user_id = %d ORDER BY created_at DESC",
        $user->ID
    )
);
...
$balance = $wpdb->get_var(
    $wpdb->prepare(
        "SELECT SUM(amount) FROM {$wpdb->prefix}blackcnote_transactions WHERE user_id = %d AND status = %s",
        $user->ID,
        'completed'
    )
) ?: 0;
...
$transactions = $wpdb->get_results(
    $wpdb->prepare(
        "SELECT * FROM {$wpdb->prefix}blackcnote_transactions WHERE user_id = %d ORDER BY created_at DESC LIMIT 5",
        $user->ID
    )
);
?>
...
<div class="blackcnote-dashboard">
    ...
    <div class="row mb-4">
        <div class="col-md-4">
            <div class="card">
                <div class="card-body">
                    <h5 class="card-title"><?php esc_html_e('Account Balance', 'blackcnote'); ?></h5>
                    <h2 class="card-text"><?php echo esc_html(number_format($balance, 2)); ?></h2>
                    <p class="card-text text-muted"><?php esc_html_e('Available for withdrawal', 'blackcnote'); ?></p>
                </div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="card-text text-muted"><?php esc_html_e('Total active plans', 'blackcnote'); ?></p>
                </div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="card">
                <div class="card-body">
                    <h5 class="card-title"><?php esc_html_e('Member Since', 'blackcnote'); ?></h5>
                    <h2 class="card-text"><?php echo esc_html(date_i18n('M Y', strtotime($user->user_registered))); ?></h2>
                    <p class="card-text text-muted"><?php esc_html_e('Account creation date', 'blackcnote'); ?></p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <?php else : ?>
                        <p class="text-muted"><?php esc_html_e('No active investments found.', 'blackcnote'); ?></p>
                    <?php endif; ?>
                </div>
            </div>
        </div>
                    ...
                    <?php if (!empty($transactions)) : ?>
                        ...
                        <div class="text-end mt-3">
                            <a href="<?php echo esc_url(get_permalink(get_page_by_path('transactions'))); ?>" class="btn btn-link">
                                <?php esc_html_e('View All Transactions', 'blackcnote'); ?>
                            </a>
                        </div>
                    <?php else : ?>
                        <p class="text-muted"><?php esc_html_e('No recent transactions found.', 'blackcnote'); ?></p>
                    <?php endif; ?>
                </div>
            </div>
        </div>
