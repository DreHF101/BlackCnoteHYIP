...
class BlackCnote_Validator {
    ...
    private array $required_files = [
        ...
        'assets/css/blackcnote.css',
        'assets/js/blackcnote.js',
        'languages/blackcnote.pot',
        'tests/test-blackcnote.php',
        'README.md',
        'CHANGELOG.md',
        'LICENSE.txt',
        'screenshot.txt',
    ...
    private array $required_functions = [
        'blackcnote_setup',
        'blackcnote_scripts',
        'blackcnote_register_post_type',
        'blackcnote_add_settings_page',
    ...
    /**
     * Check BlackCnote integration
     */
    private function check_blackcnote_integration(): void {
        // Check if BlackCnote plugin is active
        if (!is_plugin_active('blackcnote/blackcnote.php')) {
            error_log('BlackCnote plugin is not active');
            return;
        }

        // Check for required BlackCnote functions
        $required_blackcnote_functions = [
            'blackcnote_get_plans',
            'blackcnote_get_transactions',
            'blackcnote_calculate_return'
        ];

        foreach ($required_blackcnote_functions as $function) {
            if (!function_exists($function)) {
                error_log("Missing required BlackCnote function: {$function}");
            }
        }

        // Check for required BlackCnote hooks
        $required_blackcnote_hooks = [
            'blackcnote_before_dashboard',
            'blackcnote_after_dashboard',
            'blackcnote_before_plans',
            'blackcnote_after_plans',
        ...
        foreach ($required_blackcnote_hooks as $hook) {
            if (!has_action($hook)) {
                error_log("Missing required BlackCnote hook: {$hook}");
            }
        }
    }
}
...
if (defined('WP_DEBUG') && WP_DEBUG) {
    $validator = new BlackCnote_Validator();
    $validator->validate();
} 