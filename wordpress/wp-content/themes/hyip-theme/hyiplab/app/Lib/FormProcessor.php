        ...
        if ($forms) {
            for ($i=0; $i < count($forms['form_label']); $i++) {
                ...
                if ($extensions != 'null' && $extensions != null) {
                    ...
                    if ($notMatchedExt > 0) {
                        throw new \Exception( esc_html__("Your selected extensions are invalid", HYIPLAB_PLUGIN_NAME) );
                    }
                }
                $label = hyiplab_title_to_key(sanitize_text_field($forms['form_label'][$i]));
                $formData[$label] = [
        ...
        foreach($formData as $data){
            ...
            if($data->type == 'file') {
                if($request->hasFile($name)){
                    ...
                    $path = hyiplab_file_path('verify').'/'.$directory;
                    $value = $directory.'/'.hyiplab_file_uploader($value, $path);
                }else{
                    $value = null;
                }
            }
